...
class Node:
    ...
    # -------------------------------------------------------------
    # MODIFICACION: Metodo LT para que la PriorityQueue pueda comparar
    # nodos en caso de empate de costes.
    # -------------------------------------------------------------
...
def graph_search(problem, fringe):
    ...
    while fringe:
        ...
        # Incrementamos nodos visitados al analizar el nodo
        visitados += 1
        ...
        if node.state not in closed:
            ...
            # Incrementamos generados al expandir
            generados += len(successors)
            
            fringe.extend(successors)
            
    # Si no hay solucion, imprimimos las metricas hasta el momento
    print(f"   [METRICAS] No sol. Generados: {generados} | Visitados: {visitados}")
    return None
...
def branch_and_bound_graph_search(problem):
    """
    Ramificación y Acotación (Uniform Cost Search).
    Ordena la lista abierta segun el coste acumulado (g).
    """
    # PriorityQueue con min (menor coste primero) usando node.path_cost
    return graph_search(problem, PriorityQueue(min, lambda node: node.path_cost))

def astar_search(problem, h=None):
    """
    A* Search (Ramificación y Acotación con Subestimación).
    Ordena la lista abierta segun f(n) = g(n) + h(n).
    """
    h = h or problem.h
    # PriorityQueue usando node.path_cost + heuristica
    return graph_search(problem, PriorityQueue(min, lambda node: node.path_cost + h(node)))
